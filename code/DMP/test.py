...
if __name__ == "__main__":
    ...
    plt.plot(y_demo[2,:], 'p', label='demo z')
    plt.plot(y_reproduce[:,2], 'y--', label='reproduce z')
    plt.plot(y_reproduce_2[:,2], 'y-.', label='reproduce 2 z')
    plt.legend()
    plt.grid()
    plt.xlabel('time')
    plt.ylabel('y')
    ...
    ax1.plot3D(y_demo[0,:],y_demo[1,:],y_demo[2,:],'r')    #绘制空间曲线
